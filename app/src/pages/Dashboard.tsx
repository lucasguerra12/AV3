...
export function Dashboard() {
  // FIX CRÍTICO: Definindo [] como padrão caso o contexto ainda não tenha carregado
  // Isso impede o React de dar crash (White Screen of Death)
  const { logs = [], aeronaves = [] } = useSystem();  
  
  const totalAeronaves = aeronaves.length;
  
  // Usamos ?. (optional chaining) para evitar erros caso pecas ou testes não existam ainda
  const pecasEmTransito = aeronaves.reduce((total, aero) => {
    const emTransito = aero.pecas?.filter(p => p.status === 'EM_TRANSPORTE').length || 0;
    return total + emTransito;
  }, 0);

  const testesAprovados = aeronaves.reduce((total, aero) => {
    return total + (aero.testes?.filter(t => t.resultado === 'APROVADO').length || 0);
  }, 0);

  const testesReprovados = aeronaves.reduce((total, aero) => {
    return total + (aero.testes?.filter(t => t.resultado === 'REPROVADO').length || 0);
  }, 0);
          ...
          <div className="lg:col-span-2 bg-surface-low border border-outline-variant/20 rounded-sm p-6 shadow-md">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-outline-variant/10">
                ...
                {ultimasAeronaves.length === 0 && (
                   <tr><td colSpan={3} className="py-4 text-center text-sm text-on-surfaceVariant">Nenhuma aeronave sincronizada do banco de dados.</td></tr>
                )}
              </tbody>
            </table>
          </div>
