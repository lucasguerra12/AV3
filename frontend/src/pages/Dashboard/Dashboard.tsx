...
// 1. Importar o StatusEtapa
import { TipoAeronave, NivelPermissao, StatusEtapa } from '../../models/enums'; 
...
const Dashboard = ({ currentUser, aeronaves, onAdicionarAeronave }: DashboardProps) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // 2. CORREÇÃO: Usar um objeto (interface) em vez de uma 'class'
        const novaAeronave: Aeronave = {
            id: 0, // O ID será definido pelo backend, 0 é temporário
            codigo,
            modelo,
            tipo,
            capacidade,
    ...
    // 3. CORREÇÃO: Usar o enum 'StatusEtapa.CONCLUIDA'
    // E verificar se 'etapas' existe (boa prática)
    const totalEmAndamento = aeronaves.length;
    const totalPecas = aeronaves.reduce((acc, a) => acc + (a.pecas ? a.pecas.length : 0), 0);
    const totalConcluidas = aeronaves.filter(a => 
        a.etapas && a.etapas.length > 0 && a.etapas.every(e => e.status === StatusEtapa.CONCLUIDA)
    ).length;
